...
export default function PeopleAwardsPage() {
  const [items, setItems] = useState<PeopleAwardPresentationItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/people-awards", { cache: "no-store" });
      if (!response.ok) {
        throw new Error("Could not load awards");
      }
      const data = (await response.json()) as { items: PeopleAwardPresentationItem[] };
      setItems(data.items);
    } catch {
      setError("Unable to load people awards. Check the connection and try again.");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const el = document.getElementById(`people-award-slot-${activeIndex}`);
    ...
    <main className="page-shell team-awards-page">
      <header className="team-awards-header">
        ...
        <p className="muted small">
          Reveal each category in order. Winners follow audience nominations (top votes per category).
        </p>
      ...
      {loading ? (
        ...
        <div className="team-awards-stack">
          {items.map((item, i) => {
            ...
            return (
              <PersonAwardCeremonyCard
                key={item.key}
                slotId={`people-award-slot-${i}`}
                title={item.title}
